...
<?php
	...
	if (!empty($stage->clusters->compulsory)) {
		$module_clusters = array_merge($module_clusters, $stage->clusters->compulsory);
	}
	if (!empty($stage->clusters->optional)) {
		$module_clusters = array_merge($module_clusters, $stage->clusters->optional);
	}
	...
	foreach ($module_clusters as $cluster) {
		if (cluster_contains_opional_modules($cluster)) {
...
<p>TODO: Optional free text here</p>
	...
	<?php if (!empty($optional_clusters)): ?>
